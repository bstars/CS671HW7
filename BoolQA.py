...
from transformers import RagRetriever, RagTokenizer, RagModel, RagConfig, AutoTokenizer
...
class BoolQADataset(Dataset):
	def __init__(self, tokenizer, rag, include_gold:bool, split='train',):
		super().__init__()
		...
		self.tokenizer = AutoTokenizer.from_pretrained("facebook/rag-token-base")
		retriever = RagRetriever.from_pretrained(
			"facebook/rag-token-base", index_name="exact", use_dummy_dataset=True
		)
		# initialize with RagRetriever to do everything in one forward call
		self.rag = RagModel.from_pretrained("facebook/rag-token-base", retriever=retriever)
...
if __name__ == '__main__':
	...
	# initialize with RagRetriever to do everything in one forward call
	rag = RagModel.from_pretrained("facebook/rag-token-base", retriever=retriever)
	
	train_ds = BoolQADataset(tokenizer, rag, include_gold=True, split='train')
	val_ds = BoolQADataset(tokenizer, rag, include_gold=True, split='validation')
	
	train(train_ds, val_ds, lr=1e-4, epochs=10)



	# dataset = load_dataset("boolq")
	# dataset = dataset['train']['passage']
	# d = dataset['train']['passage']
